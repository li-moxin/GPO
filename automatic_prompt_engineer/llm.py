...
class GPT_Forward(LLM):
            ...
            while response is None:
                try:
                    response = openai.ChatCompletion.create(
                        **config, messages=[{"role": "user", "content": prompt[i]}])
                    ...
                    print('Retrying...')
                    time.sleep(5)
            ret_response.extend([response['choices'][j]['message']['content'] for j in range(len(response['choices']))])
        return ret_response
    ...
    def __complete(self, prompt, n):
        ...
        while response is None:
            try:
                # response = completions_with_backoff(**config, prompt=prompt)
    ...
    def __log_probs(self, text, log_prob_range=None):
        ...
        while response is None:
            try:
                # response = completions_with_backoff(**config, prompt=text)
                response = openai.Completion.create(
                    **config, prompt=text)
            except Exception as e:
                print(e)
...
class GPT_Insert(LLM):
    ...
    def __generate_text(self, prompt, n):
        ...
        while response is None:
            try:
                # response = completions_with_backoff(**config, prompt=prefix, suffix=suffix)
                response = openai.Completion.create(
